...
machine_init (struct statics *statics)
{
    ...
    */
    ...
    /* hash table for remembering procs sized based on current nproc*/
...
#define ORDERKEY_STATE   if ((result = (long) (sorted_state[(int)p2->pr_state] - \
...
getptable (struct prpsinfo *baseptr)
{
	 ...
	 * ALSO NOTE: the pctcpu recorded in the proc structure does not
	 * produce the kind of results we really want, so we don't use
	 * it even though its there.
	 */
	op = (struct oldproc *)hash_lookup_pid(prochash, currproc->pr_pid);
	if (op == NULL)
	{
	    /* new entry */
#ifdef PROC_HAS_PCTCPU
	...
	if (op == NULL)
	{
	    ...
	    hash_add_pid(prochash, currproc->pr_pid, (void *)op);
	}
    ...
    /* scan the hash table and remove dead entries */
